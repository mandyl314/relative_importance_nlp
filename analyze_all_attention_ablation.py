...
def extract_human_importance(dataset):
    with open("results_attention/" + dataset + "_sentences.txt", "r") as f:
    ...
    human_importance = []
    with open("results_attention/" + dataset + "_relfix_averages.txt", "r") as f:
...
def extract_model_importance(dataset, model, importance_type, layer = None, head = None):
    ...
    if importance_type == "attention":
        if layer != None:
            with open("results_attention/" + dataset + "_" + model + "_layer_" + str(layer) + "_" + importance_type + ".txt", "r") as f:
                for line in f.read().splitlines():
            ...
            with open("results_attention/" + dataset + "_" + model + "_head_" + str(head) + "_" + importance_type + ".txt", "r") as f:
                for line in f.read().splitlines():
                    tokens, heat = line.split("\t")
    ...
    # saliency
        ...
        with open("results_attention/" + dataset + "_" + model +"_" + importance_type + ".txt", "r") as f:
            for line in f.read().splitlines():
                tokens, heat = line.split("\t")
...
def compare_importance(et_tokens, human_salience, lm_tokens, lm_salience, importance_type, layer=None, head=None):
    count_tok_errors = 0
    ...
    if importance_type == "attention":
        if layer:
            out = "results_attention/correlations/" + corpus + "_" + model + "_layer_" + str(layer) + "_" + importance_type + "_correlations.txt"
        else:
            out = "results_attention/correlations/" + corpus + "_" + model + "_head_" + str(head) + "_" + importance_type + "_correlations.txt"
    else:
        out = "results_reproduced/correlations/" + corpus + "_" + model + "_" + importance_type + "_correlations.txt"
...
results = pd.DataFrame(columns=('importance_type', 'corpus', 'model', 'layer/head', 'mean_correlation', 'std_correlation'))
permutation_results = pd.DataFrame(
    columns=('importance_type', 'corpus', 'model', 'layer/head','mean_correlation', 'std_correlation'))
        ...
        for model in models:
            ...
            if importance_type == "attention":
                ...
                for layer in range(num_layers):
                    lm_tokens, lm_importance = extract_model_importance(corpus, model, importance_type, layer=layer)

                    # Model Correlation
                    spearman_mean, spearman_std = compare_importance(et_tokens, human_importance, lm_tokens, lm_importance, importance_type, layer=layer)
                    results = results.append( {'importance_type': importance_type, 'corpus': corpus, 'model': model, 'layer/head': f'layer {layer}', 'mean_correlation': spearman_mean, 'std_correlation': spearman_std}, ignore_index=True)
                    ...
                    permutation_results = permutation_results.append(
                        {'importance_type': importance_type, 'corpus': corpus, 'model': model, 'layer/head': f'layer {layer}', 'mean_correlation': spearman_mean, 'std_correlation': spearman_std},
                        ignore_index=True)
                    
                for head in range(num_heads):
                    print(f"Here {head}")
                    lm_tokens, lm_importance = extract_model_importance(corpus, model, importance_type, head=head)

                    # Model Correlation
                    spearman_mean, spearman_std = compare_importance(et_tokens, human_importance, lm_tokens, lm_importance, importance_type, head=head)
                    results = results.append( {'importance_type': importance_type, 'corpus': corpus, 'model': model, 'layer/head': f'head {head}','mean_correlation': spearman_mean, 'std_correlation': spearman_std}, ignore_index=True)

                    #Permutation Baseline
                    spearman_mean, spearman_std = calculate_permutation_baseline(human_importance, lm_importance)
                    permutation_results = permutation_results.append(
                        {'importance_type': importance_type, 'corpus': corpus, 'model': model, 'layer/head': f'head {head}', 'mean_correlation': spearman_mean, 'std_correlation': spearman_std},
                        ignore_index=True)


    # Store results
    with open("results_attention/all_results5.txt", "w") as outfile:
...
# i think plots are only for saliency
