...
def compute_sensitivity(model, embedding_matrix, tokenizer, text):
    ...
    for masked_token_index in range(len(token_ids)):
        ...
        # Get the actual token
            ...
            total_gradients = np.zeros_like(token_ids_tensor_one_hot, dtype=np.float32)
            ...
            for _ in range(nsamples):
                ...
                total_gradients += tape.gradient(predict_mask_correct_token, token_ids_tensor_one_hot)

            sensitivity_non_normalized = total_gradients / nsamples
